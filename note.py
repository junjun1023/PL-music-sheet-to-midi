...
class Note:
    ...
    def __init__(self, staff_no, staffs, blob, clef):  #note會有的一些值
        ...
        self.position_on_staff = self.detect_position_on_staff(staffs[staff_no], blob)  #樂譜中的位置
    ...
    def detect_position_on_staff(self, staff, blob):
        ...
        for line_no, line in enumerate(staff.lines_location):
            ...
            distances_from_lines.append((2 * line_no, distance((x, y), (x, line))))  #跟五條線的距離
        ...
        distances_from_lines = sorted(distances_from_lines, key=lambda tup: tup[1])  #把距離排序


        if distances_from_lines[1][1] - distances_from_lines[0][1] <= NOTE_PITCH_DETECTION_MIDDLE_SNAPPING:  #看距離最近的兩條線相減有沒有小於間的距離

            
            return int((distances_from_lines[0][0] + distances_from_lines[1][0]) / 2) #在線之間

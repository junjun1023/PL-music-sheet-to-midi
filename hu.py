...
def get_clef(image, staff):
    """
        ...
        """
    i = 0
    width = image.shape[0]      # shape[0]代表第二维长度(宽度) https://blog.csdn.net/u010758410/article/details/71554224
...
def hu_moments():
    ...
    """
        first compute the 24 moments with cv2.moments then pass the moments to HuMoments
        HuMoments calculate 7 invariant moments
        flatten() array to form shape feature vector
        """
...
# It takes as input the central moments of the image which can be calculated
def log_transform_hu(hu_moment):
    """
        Transforms hu moments so they can be easily compared.
        """
    # compared 传入的音符和自己本身拥有的音符做比较
...
def classify_clef(image, staff):
    ...
    # 判断 test moment跟哪个moments(voilin/bass)比较像(less diffrence)
